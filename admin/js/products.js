		...
		$.ajax({
			...
			success: function (response) {
				...
				if (resp.status == 202) {
					...
					if (productList) {
						$.each(resp.message.products, function (index, value) {
							...
							productHTML += '<tr class="tabData">' +
								...
								'<td><a class="btn btn-sm btn-info edit-product tabBttns" style="color:#fff;"><span style="display:none;">' + JSON.stringify(value) + '</span>Edit</a>&nbsp;<a pid="' + value.product_id + '" class="btn btn-sm btn-danger delete-product tabBttns">Delete</i></a></td>' +
								'</tr>';
					...
					var brandSelectHTML = '<option value="">Select Collection</option>';
