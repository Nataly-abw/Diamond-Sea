...
class Credentials
{
	...
	public function createAdminAccount($name, $email, $password){
		...
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> 'This Email Address already exists.'];
		} else {
			$password = password_hash($password, PASSWORD_BCRYPT, ["COST"=> 8]);
			$q = $this->con->query("INSERT INTO `admin`(`name`, `email`, `password`, `is_active`) VALUES ('$name','$email','$password','0')");
			if ($q) {
				return ['status'=> 202, 'message'=> 'New admin is created.'];
			}
		}
	}
	...
	public function loginAdmin($email, $password){
		...
		if ($q->num_rows > 0) {
			...
			if (password_verify($password, $row['password'])) {
				...
				return ['status'=> 202, 'message'=> 'Login succeeded for user.'];
			} else {
				return ['status'=> 303, 'message'=> 'Login Failed.'];
			}
		} else {
			return ['status'=> 303, 'message'=> 'The Email Address or Password you entered is incorrect.'];
		}
	}
}
...
if (isset($_POST['admin_register'])) {
	...
	if (!empty($name) && !empty($email) && !empty($password) && !empty($cpassword)) {
		if ($password == $cpassword) {
			...
			echo json_encode(['status'=> 303, 'message'=> 'Password doesn&apos;t match.']);
			exit();
		}
	} else {
